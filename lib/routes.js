...
exports.api = function(req, res){
    ...
    if (aUrl.pathname === '/api/wastepoint' && req.method === 'POST' ){
        ...
        var id = aUrl.pathname.substring(aUrl.pathname.lastIndexOf('/'));
        if (id) {
            api.getWastePoints(aUrl.params, sendObjectOrError);
        }
        else {
            api.getWastePoint({id: id}, sendObjectOrError);
        }
    }
    else if (aUrl.pathname === '/api/wastepointimage' && req.method === 'GET') {
        api.getWastePointImage(aUrl.params, sendObjectError);
    }
    ...
    else if (aUrl.pathname === '/api/markasdone' && req.method === 'POST') {
        api.markAsDone(req.body, sendObjectOrError);
    }
    else{
        res.statusCode = 404;
        res.send('');
